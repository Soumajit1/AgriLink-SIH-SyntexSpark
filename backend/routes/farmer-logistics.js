...
// =====================================================
// GET UNSHIPPED TRANSACTIONS FOR A FARMER
// (Transactions that don't have a shipment yet)
// =====================================================

router.get('/:farmerId/unshipped', async (req, res) => {
    try {
        const farmerId = Number(req.params.farmerId);
        ...
        const transactions = await query(sql, [farmerId]);
        ...
        console.error('Get unshipped transactions error:', error);

        return res.status(500).json({
            error: 'Failed to load unshipped transactions',
            details: error.message,
            code: error.code
        });
    }
...
router.post('/create', async (req, res) => {
    try {
        ...
        if (
            ...
            return res.status(400).json({
                error: 'Shipment cannot be created for this transaction',
        ...
        const finalTracking = tracking_number && String(tracking_number).trim()
        ...
        const result = await query(
            ...
            [
                ...
                transporter ? String(transporter).trim() : null,
                finalTracking,
...
router.post('/:shipmentId/status', async (req, res) => {
    try {
        ...
        const farmerIdValue = farmer_id ?? farmerId ?? userId;
        const validFarmerId = Number(farmerIdValue);

        if (!Number.isInteger(validFarmerId) || validFarmerId <= 0) {
            return res.status(400).json({
        ...
        const shipments = await query(
            `
            SELECT s.id, s.status, t.farmer_id
            FROM shipments s
            INNER JOIN transactions t
                ON s.transaction_id = t.id
            WHERE s.id = ?
            ...
            LIMIT 1
            `,
            [shipmentId, validFarmerId]
        );

        if (shipments.length === 0) {
            return res.status(404).json({
                error: 'Shipment not found for this farmer'
            });
        }

        const currentStatus = shipments[0].status;

        if (currentStatus === 'delivered' && status !== 'delivered') {
            return res.status(400).json({
                error: 'Delivered shipment cannot be reverted'
            });
        }
        ...
        if (transporter) {
            updateSql += ', transporter = ?';
            params.push(String(transporter).trim());
        }

        if (tracking_number) {
            updateSql += ', tracking_number = ?';
            params.push(String(tracking_number).trim());
        }
...
router.get('/shipment/:shipmentId', async (req, res) => {
    try {
        ...
        const shipments = await query(sql, [shipmentId]);
