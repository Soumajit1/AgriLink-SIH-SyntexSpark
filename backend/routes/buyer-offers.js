...
// =====================================================
// GET BUYER OFFER STATS
// GET /api/buyer/offers/:buyerId/stats
// =====================================================
...
router.get('/:buyerId/stats', (req, res) => {
    ...
    const sql = `
        SELECT
            ...
            SUM(
                CASE
                    WHEN LOWER(o.status) IN ('pending', 'countered', 'negotiating')
                    THEN 1
                    ELSE 0
                END
            ) AS pending,
            SUM(
                CASE
                    WHEN LOWER(o.status) IN ('accepted', 'completed')
                    THEN 1
                    ELSE 0
                END
            ) AS accepted,
            SUM(
                CASE
                    WHEN LOWER(o.status) IN ('rejected', 'cancelled')
                    THEN 1
                    ELSE 0
                END
            ) AS rejected
    ...
    db.query(sql, [buyerId], (err, results) => {
        ...
        if (err) {
            console.error('Offer statistics error:', err);
            return res.status(500).json({
                error: 'Failed to load offer statistics',
                details: err.message
            });
        }
...
router.get('/:buyerId', (req, res) => {
    ...
    const sql = `
        SELECT
            ...
            COALESCE(p.crop_name, 'Produce Item') AS crop_name,
            COALESCE(p.unit, 'kg') AS unit,
            COALESCE(p.price_per_unit, o.offered_price) AS price_per_unit,
            COALESCE(p.quality, 'Standard') AS quality,
            p.farmer_id,

            COALESCE(u.name, 'Farmer') AS farmer_name,
